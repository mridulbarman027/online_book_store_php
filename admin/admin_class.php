...
class Action
{
	...
	function get_cart_count()
	{
		...
		if (isset($_SESSION['login_id'])) {
			$loginIdTemp = $_SESSION['login_id'];

			$qry = $this->db->query("SELECT c.*,b.image_path,b.title FROM cart c inner join books b on b.id = c.book_id where c.customer_id ={$loginIdTemp}");
			$count = 0;
			$data['list'] = array();
